...
def process_data():
    ...
    # 제외시킬 한국인 학자/작가 이름 목록 (키워드 빈도 상위권 위주)
    exclude_names = {'황순원', '손창섭', '김수영', '조가경', '박종홍', '윤동주', '김은국', '고석규', '나종석', '오제은', '배경열', '서유경', '조광제', '팽혜숙', '최재정', '권혁정', '이상호', '이명곤'}
    
    for keywords_str in df_keywords:
        # 단어 자르기 및 한국인 이름 필터링
        kws = [k.strip() for k in keywords_str.split(',') if k.strip() and k.strip() not in exclude_names]
        
        # 추가적으로, 이름 3글자이면서 성씨가 포함된 경우를 대략적으로 필터링하고 싶다면 아래 주석을 활용할 수 있습니다.
        # kws = [k for k in kws if not (len(k) == 3 and k[0] in '김이박최정강조윤장임한오서신권황안송전홍유고문양손배조백허유남심노정하곽성차주우구신임나전민유진지엄채원천방공강현함변염양변여추노도소신석선설마길주연방위표명기반라왕금옥육진인지엄은')]
